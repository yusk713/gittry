...
def main():
    Filepath ="D:/code/dan Java/data/3dsnew/Grid Spectroscopy005.3ds"
    # Filepath = filedialog.askopenfilename() 
    file=open(Filepath,"rb")
    location=Filepath.rfind("/")
    
    name=Filepath[0:location+1]
    print(Filepath)
    ...
    for i in range(0,nx):
        for j in range(0,ny):
            if count!=0:
                try:
                    data1=file.read(4)
                    one=np.float64(struct.unpack(">f",data1))
                    data2=file.read(4)
                    two=np.float64(struct.unpack(">f",data2))
                except Exception:
                    print("over")
                    print(count,i,j)
                    print(data1)
    ...
    #         for j in range(5):
    #             for k in range(5):
    #                 print(dataset[0][i][j][k])
        
    
    
    file.close()
    
    topobinfile=open(str(name)+"3ds.bin","wb")
    data=struct.pack(">i",nx)#java和python里的int都是4个bytes
    ...
    for i in range(nchannels):
        ...
        if channel[i].find("/")==1:
            channel[i]=channel[i].split("/")[0]+channel[i].split("/")[1]
        binfile=open(str(name)+channel[i]+".bin","wb")
